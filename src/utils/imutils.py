...
def load_image_ori(img_path):
    # H x W x C => C x H x W
    return im_to_torch(np.array(imageio.imread(img_path, pilmode='RGB')))


def custom_resize(img, owidth, oheight):
    img = im_to_numpy(img)
...
def show_sample(inputs, target):
    ...
    for n in range(num_sample):
        inp = custom_resize(inputs[n], width, height)
        out = inp
