...
from src.utils.utils_augment import Augment
...
def generate_data(percentage, stage, animal_list):
    ...
    for animal in animal_list:
        if isfile(
            ...
            ssl_kpts_data = np.load(
                './animal_data/psudo_labels/stage{}/all/ssl_labels_train_{}.npy'.format(stage, animal),
                allow_pickle=True)
...
def get_warpmat(r, s):
    ...
    M = cv2.getRotationMatrix2D(center=(32, 32), angle=float(r), scale=1 / float(s))
...
class Real_Animal_Crop_All(data.Dataset):
    ...
    def load_animal(self, percentage, stage):
        ...
        for animal in self.animal:
            n_sample = 0
            img_list = glob.glob(
                os.path.join(self.img_folder, 'real_animal_crop_v4', 'real_' + animal + '_crop', '*.jpg'))
            img_list = sorted(img_list)
            train_idxs = np.load('./data/real_animal/' + animal + '/train_idxs_by_video.npy')

            if isfile(
                    './animal_data/psudo_labels/stage{}/all/ssl_labels_train_p{}_{}.npy'.format(stage, percentage,
                                                                                                animal)):
                print("==> load ssl labels from {} stage{} percentage {}".format(animal, stage, percentage))
                ssl_labels_list = np.load(
                    './animal_data/psudo_labels/stage{}/all/ssl_labels_train_p{}_{}.npy'.format(stage,
                                                                                                percentage, animal),
                    allow_pickle=True)
