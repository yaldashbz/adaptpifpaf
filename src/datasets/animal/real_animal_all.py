...
class Real_Animal_All(data.Dataset):
    def __init__(self, is_train=True, is_aug=False, **kwargs):
        ...
        self.animal = ['horse', 'tiger'] if kwargs['animal'] == 'all' else [
            kwargs['animal']]  # train on single or all animal categories
    ...
    def __getitem__(self, index):
        ...
        for i in range(nparts):
            if tpts[i, 1] > 0:
                ...
                tpts_inpres[i, 0:2] = to_torch(
                    transform(tpts_inpres[i, 0:2] + 1, c, s, [self.inp_res, self.inp_res], rot=r))
